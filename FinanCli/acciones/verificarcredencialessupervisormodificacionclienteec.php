						...
						if ($stmt40 = $mysqli->prepare("SELECT id FROM finan_cli.consulta_estado_financiero WHERE token = ? AND tipo_documento = ? AND documento = ? AND validado = 0")) 
						{
							...
							if($stmt40->num_rows > 0)
							{
								...
								else
								{
									...
									if(!$stmt43 = $mysqli->prepare("INSERT INTO finan_cli.estado_cliente(fecha,tipo_documento,documento,id_motivo,usuario,usuario_supervisor) VALUES (?,?,?,?,?,?)"))
									{
										echo $mysqli->error;
										$mysqli->autocommit(TRUE);
										$stmt->free_result();
									...
									else
									{
										...
										$stmt43->bind_param('sisiss', $date_registro_a_eccef_db, $tipoDocumento, $documento, $motivo, $_SESSION['username'], $usuarioSupervisor);
										if(!$stmt43->execute())
										{
											echo $mysqli->error;
											$mysqli->autocommit(TRUE);
