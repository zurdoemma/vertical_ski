<?php 		
		...
		$montoIntereses = 0;
		if($stmt63 = $mysqli->prepare("SELECT SUM(mcc.monto_interes) FROM ".$db_name.".mora_cuota_credito mcc, ".$db_name.".cuota_credito cc WHERE mcc.id_cuota_credito = cc.id AND cc.id_credito = ?"))
		{
		...
		echo '						<input class="form-control input-sm" id="montototalcreditvi" name="montototalcreditvi" type="text" maxlength="11" value="'.str_replace(",",".",round(($montoTotalConInteresesCredito/100.00),2)).'" disabled />';
		echo '					 </div>';
		...
		echo '						<input class="form-control input-sm" id="interesescreditvi" name="interesescreditvi" type="text" maxlength="11" value="'.str_replace(",",".",round(($montoIntereses/100.00),2)).'" disabled />';
		echo '					</div>';
											...
											<div id="tablefeescreditclientv" class="table-responsive">
												<table id="tablefeescreditclienttv" data-classes="table table-hover table-condensed"
													...
													<tbody>';													
														...
														while($stmt62->fetch())
														{		
															if($stmt64 = $mysqli->prepare("SELECT SUM(mcc.monto_interes) FROM ".$db_name.".mora_cuota_credito mcc, ".$db_name.".cuota_credito cc WHERE mcc.id_cuota_credito = cc.id AND cc.id_credito = ? AND cc.id = ?"))
															{
																...
																if($estado_cuota_db == translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']) || $estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang']))
																{
																	...
																	if($pasoPrimeraCuota == 0)
																	{
																		...
																		if($monto_interes_cuota_credito_db == 0)
																		{
																			if($totR105 > 0)
																			{
																				if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button id="pagoCuotaNro'.$numero_cuota_db.'" type="button" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Payment_Fee_Credit',$GLOBALS['lang']).'" onclick="pagarCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-cash-register"></i></button>&nbsp;<button type="button" id="btnCambiarEstadoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Change_State_Fee_Credit',$GLOBALS['lang']).'" onclick="cambiarEstadoCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-sign-in-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																				else echo '<td><button id="pagoCuotaNro'.$numero_cuota_db.'" type="button" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Payment_Fee_Credit',$GLOBALS['lang']).'" onclick="pagarCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-cash-register"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																			}
																			else
																			{	
																		...
																		else
																		{
																			if($totR105 > 0)
																			{	
																				if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button id="pagoCuotaNro'.$numero_cuota_db.'" type="button" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Payment_Fee_Credit',$GLOBALS['lang']).'" onclick="pagarCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-cash-register"></i></button>&nbsp;<button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnCambiarEstadoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Change_State_Fee_Credit',$GLOBALS['lang']).'" onclick="cambiarEstadoCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-sign-in-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																				else echo '<td><button id="pagoCuotaNro'.$numero_cuota_db.'" type="button" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Payment_Fee_Credit',$GLOBALS['lang']).'" onclick="pagarCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-cash-register"></i></button>&nbsp;<button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																			}
																			else
																			{
																				if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button id="pagoCuotaNro'.$numero_cuota_db.'" type="button" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Payment_Fee_Credit',$GLOBALS['lang']).'" onclick="pagarCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-cash-register"></i></button>&nbsp;<button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnCambiarEstadoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Change_State_Fee_Credit',$GLOBALS['lang']).'" onclick="cambiarEstadoCuotaCredito('.$idCredito.','.$id_cuota_credito_db.')"><i class="fas fa-sign-in-alt"></i></button></td>';
																	...
																	else
																	{
																		...
																		if($monto_interes_cuota_credito_db == 0)
																		{
																			//if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button type="button" id="btnCambiarEstadoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Change_State_Fee_Credit',$GLOBALS['lang']).'" onclick="cambiarEstadoCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-sign-in-alt"></i></button></td>';
																			//else echo '<td>---</td>';
																			if($totR105 > 0)
																			{
																				echo '<td><button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																			}
																			else
																			{
																				echo '<td>---</td>';
																		...
																		{
																			//if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnCambiarEstadoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Change_State_Fee_Credit',$GLOBALS['lang']).'" onclick="cambiarEstadoCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-sign-in-alt"></i></button></td>';
																			if($totR105 > 0)
																			{
																				if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																				else echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';																				
																			}
																			else
																			{
																				if($estado_cuota_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button></td>';
																				else echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button></td>';
																			}
																		}																		
																	}
																...
																else
																{
																	...
																	if($monto_interes_cuota_credito_db == 0)
																	{
																		...
																		{	
																			...
																			{
																				if(($estado_credito_db_res == translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang'])) && $numero_cuota_db == $ultimo_numero_cuota_pagada_db && ($_SESSION["permisos"] == 1 || $_SESSION["permisos"] == 3))
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																					else echo '<td><button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';																					
																				}
																				else
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																					else echo '<td><button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																				}
																			}
																			else echo '<td><button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																		}
																		else
																		{
																			if($estado_cuota_db == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang']))
																			{
																				if(($estado_credito_db_res == translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang'])) && $numero_cuota_db == $ultimo_numero_cuota_pagada_db && ($_SESSION["permisos"] == 1 || $_SESSION["permisos"] == 3))
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																					else echo '<td><button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																				}
																				else
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button></td>';
																					else echo '<td>---</td>';																					
																				}
																			}
																			else echo '<td>---</td>';																			
																	...
																	else
																	{
																		...
																		{																			
																			if($estado_cuota_db == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang']))
																			{
																				if(($estado_credito_db_res == translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang']) || $estado_credito_db_res == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang'])) && $numero_cuota_db == $ultimo_numero_cuota_pagada_db && ($_SESSION["permisos"] == 1 || $_SESSION["permisos"] == 3))
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																					else echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';																					
																				}
																				else
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																			...
																			{
																				echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnVerAvisosDeudaCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Debt_Notices',$GLOBALS['lang']).'" onclick="verAvisosDeuda('.$id_cuota_credito_db.')"><i class="fas fa-diagnoses"></i></button></td>';
																			}
																		}
																		else
																		{
																			if($estado_cuota_db == translate('Lbl_Status_Fee_Paid',$GLOBALS['lang']))
																			{
																				...
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																					else echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnCancelarPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Cancel_Pay_Fee_Credit',$GLOBALS['lang']).'" onclick="cancelarPagoCuotaCredito('.$id_cuota_credito_db.')"><i class="far fa-window-close"></i></button></td>';
																				}
																				else
																				{
																					if($totR65 == 0 || $totR66 == 0) echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="reImprimirPagoCuotaCreditoCliente('.$id_cuota_credito_db.')"><i class="fas fa-print"></i></button>&nbsp;<button type="button" id="btnReimprimirPagoCuotaPDF'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_Reprint_Payment_Amount_Fee_Credit',$GLOBALS['lang']).'" onclick="window.open(\'acciones/mostrarpdfpagocuotacredito.php?idCredito='.$idCredito.'&idCuotaCredito='.$id_cuota_credito_db.'\')"><i class="far fa-file-pdf"></i></button></td>';
																			...
																			{
																				echo '<td><button type="button" id="btnVerInteresesCuota'.$id_cuota_credito_db.'" class="btn" data-toggle="tooltip" data-placement="top" title="'.translate('Msg_View_Interest_Fee_Credit',$GLOBALS['lang']).'" onclick="verInteresesCuotaCredito('.$id_cuota_credito_db.')"><i class="fas fa-money-check-alt"></i></button></td>';
																			}																			
																		}
																	}
																	echo '<td>'.$estado_cuota_db.'</td>';																	
		...
		if($estado_credito_cliente_db == translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']) || $estado_credito_cliente_db == translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang'])) 
		{
			...
			else if($cantidadCuotasPendientes <= 1)
			{
				echo '			<input type="button" class="btn btn-primary pull-right" name="btnPagoSeleccionCD" id="btnPagoSeleccionCD" value="'.translate('Lbl_Payment_Selection_Fees_Credit',$GLOBALS['lang']).'" onClick="confirmar_accion_pago_seleccion_cuotas(\''.translate('Lbl_Confirmation_Action_Register_Client',$GLOBALS['lang']).'\',\''.translate('Msg_Be_Sure_To_Pay_Fees_Credit_Selection',$GLOBALS['lang']).'\');" style="margin-left:10px; display:none;" disabled />';				
				echo '			<input type="button" class="btn btn-primary pull-left" name="btnPagoTotalCD" id="btnPagoTotalCD" value="'.translate('Lbl_Payment_Total_Amount_Fees_Credit',$GLOBALS['lang']).'" onClick="pagoTotalDeuda('.$idCredito.');" style="margin-right:10px; display:none;" disabled />';
				echo '			<input type="button" class="btn btn-primary pull-left" name="btnReimpresionPagoTotalCD" id="btnReimpresionPagoTotalCD" value="'.translate('Lbl_Reprint_Payment_Total_Amount_Fees_Credit',$GLOBALS['lang']).'" onClick="reimprimirPagoTotalDeuda('.$idCredito.');" style="display:none; margin-right:10px;" disabled/>';
				echo '			<input type="button" class="btn btn-primary pull-left" name="btnPDFPagoTotalCD" id="btnPDFPagoTotalCD" value="'.translate('Lbl_PDF_Payment_Total_Amount_Fees_Credit',$GLOBALS['lang']).'" onClick="window.open(\'acciones/mostrarpdfpagototaldeudacredito.php?idCredito='.$idCredito.'\');" style="display:none;" disabled/>';								
			}
		}
