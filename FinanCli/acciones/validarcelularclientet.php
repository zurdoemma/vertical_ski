<?php 		
		...
		$resultado_env = envio_sms(translate('Lbl_From_SMS_ID_Sent',$GLOBALS['lang']), $prefijoTelefono.$nroTelefono, str_replace("%4",$nombre_cadena_validacion_celular,translate('Msg_Client_Registration_Phone_Verification_Code',$GLOBALS['lang'])).': '.$codigo_validacion_vcc);
		...
		{			
			...
			else
			{
				...
				echo '					<input type="button" class="btn btn-primary pull-right" name="btnValidarVSMS" id="btnValidarVSMS" value="'.translate('Lbl_OK',$GLOBALS['lang']).'" onClick="verificarValidacionSMSAltaTelefono(document.getElementById(\'formulariovsms\'));"/>';										
				echo '				</div>';				
				echo '			</form>';
				echo '		</div>';
				echo '	</div>';
