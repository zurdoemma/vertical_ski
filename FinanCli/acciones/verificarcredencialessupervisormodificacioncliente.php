			...
			if ($stmt->num_rows == 1) 
			{
				...
				else 
				{
					...
					if ($db_password == $password) 
					{
						...
						if(!$stmt10 = $mysqli->prepare("INSERT INTO finan_cli.estado_cliente(fecha,tipo_documento,documento,id_motivo,usuario,usuario_supervisor,token) VALUES (?,?,?,?,?,?,?)"))
						{
							echo $mysqli->error;
						...
						else
						{
							...
							$tokenACM = md5(uniqid(rand(), true));
							$tokenACM = hash('sha512', $tokenACM);
							$stmt10->bind_param('sisisss', $date_registro_a_s, $tipoDocumento, $documento, $motivo, $_SESSION['username'],$usuarioSupervisor,$tokenACM);
							if(!$stmt10->execute())
							{
								echo $mysqli->error;
								$mysqli->autocommit(TRUE);
