<?php 		
		...
		if($stmt = $mysqli->prepare("SELECT c.id, td.nombre, c.documento, c.nombres, c.apellidos, c.estado, CASE WHEN c.id_titular IS NOT NULL THEN '".translate('Lbl_Type_Account_Client_Additional',$GLOBALS['lang'])."' ELSE '".translate('Lbl_Type_Account_Client_Holder',$GLOBALS['lang'])."' END AS tipoCuenta FROM finan_cli.cliente c WHERE c.id = ?"))
		{
			...
			else
			{
				...
				echo '					&nbsp;&nbsp;&nbsp;&nbsp;<label class="control-label" for="tipoclient">'.translate('Lbl_Type_Client',$GLOBALS['lang']).':</label>';
				echo '					<div class="form-group" id="tipoclient">';
				echo '						<select class="form-control input-sm" name="tipoclienti" id="tipoclienti" style="width:190px;">';			 
				...
				echo '					<input type="button" class="btn btn-primary pull-right" name="btnCancelarNC" id="btnCancelarNC" value="'.translate('Lbl_Cancel',$GLOBALS['lang']).'" onClick="$(\'#dialogmodifyclient\').dialog(\'close\');" style="margin-left:10px;" />';
				echo '					<input type="button" class="btn btn-primary pull-right" name="btnCargarNC" id="btnCargarNC" value="'.translate('Lbl_Save',$GLOBALS['lang']).'" onClick="guardarNuevoCliente(document.getElementById(\'formulariomc\'));"/>';										
				echo '				</div>';				
				echo '			</form>';
				echo '		</div>';
				echo '	</div>';
