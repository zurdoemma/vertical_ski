<?php 		
		...
		if($stmt = $mysqli->prepare("SELECT d.id, d.calle, d.nro_calle, p.id, d.localidad, d.departamento, d.piso, d.codigo_postal, d.entre_calle_1, d.entre_calle_2, cd.preferido FROM finan_cli.cliente c, finan_cli.domicilio d, finan_cli.cliente_x_domicilio cd, finan_cli.provincia p WHERE d.id_provincia = p.id AND c.id = ? AND c.tipo_documento = cd.tipo_documento AND c.documento = cd.documento AND d.id = cd.id_domicilio AND d.id = ?"))
		{
			...
			{
				$stmt->bind_result($id_domicilio_client, $client_dom_calle, $client_dom_nro_calle, $client_dom_provincia, $client_dom_localidad, $client_dom_departamento, $client_dom_piso, $client_dom_codigo_postal, $client_entre_calle_1, $client_entre_calle_2, $client_preference);			
				$stmt->fetch();
				...
				echo '			<form id="formularionmdc" role="form">';
				...
				echo '						<input title="'.translate('Msg_A_Street_Must_Enter',$GLOBALS['lang']).'" class="form-control input-sm" id="callemi" name="callemi" type="text" maxlength="150" value="'.$client_dom_calle.'" />';
				echo '					</div>';							
				...
				echo '						<input title="'.translate('Msg_A_Street_Number_Must_Enter',$GLOBALS['lang']).'" class="form-control input-sm" id="nrocallemi" name="nrocallemi" type="text" maxlength="11" value="'.$client_dom_nro_calle.'" />';
				echo '					</div>';		
				...
				echo '						<select class="form-control input-sm" name="domprovinciai" id="domprovinciami" style="width:190px;">';			 
												...
												{ 
													...
													while($stmt->fetch())
													{
														if($client_dom_provincia == $id_provincia)
														{
				...
				echo '						<input title="'.translate('Msg_A_City_Must_Enter',$GLOBALS['lang']).'" class="form-control input-sm" id="domlocalidadmi" name="domlocalidadmi" type="text" maxlength="150" value="'.$client_dom_localidad.'" />';
				echo '					</div>';							
				...
				echo '						<input class="form-control input-sm" id="domdepartamentomi" name="domdepartamentomi" type="text" maxlength="10" value="'.$client_dom_departamento.'" />';
				echo '					</div>';
				...
				echo '						<input title="'.translate('Msg_A_Floor_Number_Must_Enter_A_Whole',$GLOBALS['lang']).'" class="form-control input-sm" id="domfloormi" name="domfloormi" type="text" maxlength="11" value="'.(!empty($client_dom_piso) ? "$client_dom_piso" : "").'" />';
				echo '					</div>';				
				echo '				</div>';
				echo '				<div class="form-group form-inline">';
				...
				echo '						<input class="form-control input-sm" id="zipcodemi" name="zipcodemi" type="text" maxlength="10" value="'.$client_dom_codigo_postal.'" />';
				echo '					</div>';				
				...
				echo '						<input class="form-control input-sm" id="entrecalle1mi" name="entrecalle1mi" type="text" maxlength="150" value="'.$client_entre_calle_1.'" />';
				echo '					</div>';							
				...
				echo '						<input class="form-control input-sm" id="entrecalle2mi" name="entrecalle2mi" type="text" maxlength="150" value="'.$client_entre_calle_2.'" />';
				echo '					</div>';				
				echo '				</div>';
				echo '				<div class="form-group form-inline">';
				echo '					<div id="domiciliopreferidom">';				
				echo '						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label class="control-label" for="domiciliopreferidoclientem">'.translate('Lbl_Preference_Address',$GLOBALS['lang']).':</label>';			
				echo '						<div class="form-group" id="domiciliopreferidoclientem">';	
				echo '							<label class="switch">';
				if($client_preference == 1) echo '<input type="checkbox" id="domiciliopreferidoclientemi" name="domiciliopreferidoclientemi" checked />';
				else echo '						 <input type="checkbox" id="domiciliopreferidoclientemi" name="domiciliopreferidoclientemi" />';
				...
				echo '					<input type="button" class="btn btn-primary pull-right" name="btnCargarm" id="btnCargarm" value="'.translate('Lbl_Save',$GLOBALS['lang']).'" onClick="guardarModificacionDomicilio(document.getElementById(\'formularionmdc\'),'.$idDomicilio.');"/>';										
				echo '				</div>';				
				echo '			</form>';
				echo '		</div>';
				echo '	</div>';
