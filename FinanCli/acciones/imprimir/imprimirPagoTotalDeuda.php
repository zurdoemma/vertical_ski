...
try 
{
	if($GLOBALS['tipoimpresora'] == translate('Lbl_Type_China_Print_Credit',$GLOBALS['lang']))
	{
		...
		if($esCopia == 1)
		{
			...
			$printer -> text(translate('Lbl_Print_Type_Voucher_Pay_Total_Amount_Debt_Credit',$GLOBALS['lang']));
			$printer -> feed();
			$printer -> setTextSize(1, 1);			
		...
		$printer -> feed();
		$printer -> setJustification($justification[0]);		
		$printer -> text(translate('Lbl_Amount_Original_Fees_Print_Credit',$GLOBALS['lang']).': $'.number_format(($montoTotalCuotasOrig/100.00), 2, ',', '.'));
		$printer -> feed();
		if(empty($montoTotalIntereses)) $printer -> text(translate('Lbl_Amount_Interest_Print_Credit',$GLOBALS['lang']).': $0,00');
		else $printer -> text(translate('Lbl_Amount_Interest_Print_Credit',$GLOBALS['lang']).': $'.number_format(($montoTotalIntereses/100.00), 2, ',', '.'));		
		$printer -> feed();
