<?php 		
		...
		if($stmt93 = $mysqli->prepare("SELECT cc.numero_cuota FROM ".$db_name.".cuota_credito cc, ".$db_name.".credito c, ".$db_name.".credito_cliente ccli WHERE c.id = ccli.id_credito AND c.id = cc.id_credito AND cc.id_credito = ? AND cc.estado IN (?,?) AND cc.numero_cuota IN (".$nrosCuotasCreditoCDB.")"))
		{
		...
		for($i = 0; $i < count($cuotasRecursive); $i++) 
		{
			...
			if($stmt63 = $mysqli->prepare("SELECT cc.numero_cuota, cc.monto_cuota_original, c.monto_compra, c.cantidad_cuotas, ccli.documento, ccli.tipo_documento, cc.fecha_pago, cc.monto_pago, cc.id  FROM ".$db_name.".cuota_credito cc, ".$db_name.".credito c, ".$db_name.".credito_cliente ccli WHERE c.id = ccli.id_credito AND c.id = cc.id_credito AND cc.numero_cuota = ? AND cc.id_credito = ? AND cc.estado IN (?,?)"))
			{
				$estadoU = translate('Lbl_Status_Fee_Pending',$GLOBALS['lang']);
				$estadoD = translate('Lbl_Status_Fee_In_Mora',$GLOBALS['lang']);
			...
			if($stmt83 = $mysqli->prepare("SELECT SUM(mcc.monto_interes) FROM ".$db_name.".mora_cuota_credito mcc, ".$db_name.".cuota_credito cc WHERE mcc.id_cuota_credito = cc.id AND cc.id_credito = ? AND cc.id = ?"))
			{
			...
			if(!$stmt81 = $mysqli->prepare("INSERT INTO ".$db_name.".pago_seleccion_cuotas_credito(id_cuota_credito,fecha,monto,usuario) VALUES (?,?,?,?)"))
			{
				echo $mysqli->error;
				$mysqli->rollback();
				$mysqli->autocommit(TRUE);
