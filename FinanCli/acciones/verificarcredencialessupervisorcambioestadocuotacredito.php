			...
			if ($stmt->num_rows == 1) 
			{
				...
				else 
				{
					...
					if ($db_password == $password || $ingresoToken == 1) 
					{
						if(!empty($tokenVSCE))
						{
							if($stmt650 = $mysqli->prepare("SELECT tcec.validado FROM ".$db_name.".token_cambio_estado_cuota tcec WHERE tcec.token = ? AND tcec.validado = ?"))
							{
								$validadoC = 0;
								...
								if($totR650 > 0)
								{
									...
									if(!$stmt80 = $mysqli->prepare("UPDATE ".$db_name.".token_cambio_estado_cuota SET validado = ?, usuario_supervisor = ? WHERE token = ? AND validado = ?"))
									{
										echo $mysqli->error;
										$mysqli->autocommit(TRUE);
										return;
