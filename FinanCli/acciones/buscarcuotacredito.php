<?php 		
		...
		if($stmt63 = $mysqli->prepare("SELECT cc.numero_cuota, cc.monto_cuota_original FROM ".$db_name.".cuota_credito cc WHERE cc.id = ? AND cc.id_credito = ? AND cc.estado IN (?,?)"))
		{
		...
		echo '						<input class="form-control input-sm" id="montooriginalcuotacreditvi" name="montooriginalcuotacreditvi" type="text" maxlength="11" value="'.str_replace(",",".",round(($monto_cuota_original_db/100.00),2)).'" disabled/>';
		echo '					</div>';				
		echo '				</div>';
		echo '				<div class="form-group form-inline">';
		...
		echo '						<input class="form-control input-sm" id="interescuotacreditvi" name="interescuotacreditvi" type="text" maxlength="11" value="'.str_replace(",",".",round(($monto_interes_cuota_credito/100.00),2)).'" disabled />';
		echo '					</div>';
		...
		echo '						<input class="form-control input-sm" title="'.translate('Msg_A_Total_Amount_Payment_Fee_Credit_Must_Enter',$GLOBALS['lang']).'" id="montototalcuotacreditvi" name="montototalcuotacreditvi" type="text" maxlength="11" value="'.str_replace(",",".",round((($monto_cuota_original_db+$monto_interes_cuota_credito)/100.00),2)).'" />';
		echo '					</div>';				
		echo '				</div>';
		echo '				<div class="form-group form-inline">';				
