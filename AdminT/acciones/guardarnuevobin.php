<?php 		
		...
		if($stmt2 = $mysqli->prepare("SELECT b.bin_id FROM tef.bines b WHERE b.payment_method_id IN (SELECT b2.payment_method_id FROM tef.bines b2 WHERE b2.payment_method_id = b.payment_method_id AND CAST(b2.range_from AS INT) <= ? AND CAST(b2.range_to AS INT) >= ?)"))
		{
			...
			{					
				...
				if(!$stmt10 = $mysqli->prepare("INSERT INTO tef.bines(company_id,range_from,range_to,payment_method_id,bin_length,facility_payments_allowed,create_timestamp,update_timestamp,config_version,enabled,priority) VALUES (?,?,?,?,?,?,?,?,?,?,?)"))
				{
					echo $mysqli->error;
				...
				else 
				{
					...
					$stmt10->bind_param('issiisssiii', $companyID, $rangoDesde, $rangoHasta, $idTarjeta, $largoBin, $facilityPaymentsAllowed, $createTimestamp, $updateTimestamp, $configVersion, $enableB, $priorityB);
					if(!$stmt10->execute())
					{
						echo $mysqli->error;
						$mysqli->autocommit(TRUE);
				...
				if($stmt = $mysqli->prepare("SELECT b.bin_id, pm.payment_method_id, pm.payment_method_description, b.range_from, b.range_to, b.bin_length FROM tef.bines b, tef.paymentmethods pm  WHERE b.payment_method_id = pm.payment_method_id ORDER BY CAST(b.range_from AS INT) LIMIT 5000"))
				{
					...
					echo translate('Msg_New_Bin_OK',$GLOBALS['lang']).'=:=:=:'.json_encode($array);
				}
				else 
				{
					echo translate('Msg_Unknown_Error',$GLOBALS['lang']);
